...
public class Shipment extends JFrame{
	...
	public Shipment(DB server, LinkedHashMap<Integer, Item> inventory, UI myGUI, boolean incoming) {
		...
		if(incoming) {
			...
			panel.add(new JLabel("New Outgoing Shipment"));
		}
		...
		btnCreateOrder.addActionListener(e->{
			try {
				server.createShipment(orders, inventory);
	...
	private void addToOrder(){
		...
		int qty = (Integer.parseInt(textField_2.getText().trim())) * -1;
